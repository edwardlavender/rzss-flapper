...
physio$identical_2 <- identical(
  utils.add::substr_end(physio$pit, nchar(physio$pit_f)),
  physio$pit_f
)
## Manual checking
...
fights$mesh_ID <- fvcom.tbx::find_cells(
  lat = fights$lat, long = fights$lon,
  mesh = mesh,
  proj = wgs84,
  return = 4
)
  ...
  velocities_by_date <-
    lapply(fights_by_date, function(fights_on_date) {
      ...
      if (fights_on_date$year[1] < 2019) {
        server_catalog <- paste0(
          "https://thredds.sams.ac.uk/thredds/",
      ...
      dat_with_u <- fvcom.tbx::thredds_extract(dat,
        var = "u",
        server_catalog = server_catalog
      )
      dat_with_v <- fvcom.tbx::thredds_extract(dat,
        var = "v",
        server_catalog = server_catalog
      )
...
#### Select rows/columns
fights <- fights[complete.cases(fights[, c(
  "event_id",
  "time_fight",
  "size_area",
  "current_speed",
